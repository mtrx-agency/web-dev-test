...
(richSnippetReviewsWidgets = function () {
    ...
    if (4 === this.readyState && !reviewsJsonLdRendered && 200 == this.status) {
      ...
      t.data &&
        ...
        t.company_data &&
          "WebPage" == n["@type"] &&
          ((reviews = JSON.parse(e)),
          (reviews = richSnippetRecursiveMerge(reviews, t.company_data)),
          (e = JSON.stringify(reviews))),
  ...
  for (i in t)
    "sku" == i && Array.isArray(t[i]) && (t[i] = t[i].join(";")),
      r.length > 0 && (r += "&"),
      "object" != typeof t[i]
        ? (r += i + "=" + encodeURIComponent(t[i]))
        : (r += i + "=" + encodeURIComponent(JSON.stringify(t[i])));
  ...
  if (
    ...
    (d.title = "Reviews Widget"),
    (n.innerHTML = ""),
    n.appendChild(d),
    (window.onresize = function () {
      (d.width = window.getComputedStyle(n, null).getPropertyValue("width")),
        (d.style.width =
          window.getComputedStyle(n, null).getPropertyValue("width") + "px");
    }),
    "get" == (r.length > 1e3 ? "post" : "get"))
  )
    d.setAttribute("src", o + "?" + r);
  else {
    ...
    for (var c in t) {
      ...
      (l.type = "hidden"), (l.value = t[c]), (l.name = c), s.appendChild(l);
    }
    document.body.appendChild(s), s.submit();
  }
  var p = function (t) {
      if ("richSnippetReviewsWidget-defaultContainer" == e) {
        var i = document.getElementById(e);
        hideIframe && clearTimeout(hideIframe),
          i &&
            (i.setAttribute("class", ""),
            window.pageYOffset > 20 && window.innerWidth < 650
              ? (i.style.opacity = "0")
              : (i.style.opacity = "1"),
            (hideIframe = setTimeout(function () {
              1 == i.style.opacity
                ? i.classList.remove(
                    "richSnippetReviewsWidget-defaultContainer--faded",
                  )
                : i.classList.add(
                    "richSnippetReviewsWidget-defaultContainer--faded",
                  );
      ...
      try {
        ...
        if (void 0 != n.action)
          switch (
            ...
            case "resize":
              n.elementId == e && (d.height = n.height);
              break;
            case "resize-load":
              d.height = n.height;
              break;
            case "RUKFloatingWidget" + floatingWidgetPosition + "#Ready":
              ...
              break;
            case "RUKFloatingWidget" + floatingWidgetPosition + "#Mobile":
              "richSnippetReviewsWidget-defaultContainer" == e &&
                (i.classList.add("RUKFloatingWidget--mobile"),
                window.addEventListener && addEventListener("scroll", p));
              break;
            case "RUKFloatingWidget" + floatingWidgetPosition + "#Expand":
              "richSnippetReviewsWidget-defaultContainer" == e &&
                (i.classList.add("expanded"),
                window.__fullPictureEvents
                  ? window.__fullPictureEvents.addWidget(
                      "iframe#" + e + "_frame",
                      "floating-widget",
                    )
                  : (window.__reviewsIoWidgetList ||
                      (window.__reviewsIoWidgetList = []),
                    window.__reviewsIoWidgetList.push({
                      target: "iframe#" + e + "_frame",
            ...
            case "RUKFloatingWidget" + floatingWidgetPosition + "#Retract":
              "richSnippetReviewsWidget-defaultContainer" == e &&
                ...
                window.__fullPictureEvents &&
                  window.__fullPictureEvents.fullPictureHideEvent(
                    "floating-widget",
                  ));
              break;
            case "RUKFloatingWidget" + floatingWidgetPosition + "#Dismiss":
              ...
              break;
            case "RUKFloatingWidget" + floatingWidgetPosition + "#Small":
              "richSnippetReviewsWidget-defaultContainer" == e &&
                (i.style.maxWidth = "45px");
          }
      } catch (e) {}
    };
  window.addEventListener
    ? addEventListener("message", g, !1)
    : attachEvent("onmessage", g);
}),
